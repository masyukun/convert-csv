...
/**
 * This package represents a model of a Relational Database. It parses SQL
 * schema information to populate the model. It outputs the model as different
 * things, like a pretty text table or MarkLogic sem:triples.
 * 
...
public class RDBSchemaModel {
  ...
  private static Logger logger = LogManager.getLogger(RDBSchemaModel.class
      .getClass());

  // Useful data structures
  public static enum OUTPUT_TYPES {
    PLAINTEXT, SEMTRIPLES, SQLINSERT
  };
  ...
  // "tablename/columname" -->
  // "http://matthewroyal.com/ConvertRDB/dbname/tablename#columnumber"
  ...
  public RDBSchemaModel(String schemaOutputFilename, String dbName) {
    this.schemaOutputFilename = schemaOutputFilename;
    this.dbName = dbName;
  }

  public RDBSchemaModel(String sqlFilename, String schemaOutputFilename,
      String dbName) {
    ...
    try {
      ...
      logger
          .error(
              String
                  .format(
                      "FAILED parsing URI [%s] as a URI reference. Try making it more URL-ish.",
                      newURI), e);
    }
    catch (MalformedURLException me) {
      logger.error(String.format(
          "Malformed URI [%s] has occurred. Either no legal protocol could be found "
              + "or the string could not be parsed.", newURI), me);
    }
  ...
  /**
   * Given a column key of the format "tablename/columnname", generate or
   * retrieve a unique URI for the column.
   * 
   * @param key
  ...
  private String getUniqueColumnURI(String key, String columnNamespace) {
    ...
    if (null == columnUriMap.get(key)) {
      ...
      uniqueURI = columnUriMap.get(key);
    }
  ...
  /**
   * <p>
   * Print the schema out.
   * </p>
   * <p>
   * The model should already be built; otherwise this will output nothing.
   * </p>
   * 
   * @param format
   *          The format you want the schema output as. (Required)
   * @param schemaOutputFilename
   *          Filename for the output schema file. (Optional)
   * @return
   */
  public String printSchema(OUTPUT_TYPES format, String schemaOutputFilename,
      String dbName) throws IllegalArgumentException {

    // Use the specified output filename, otherwise use the default
    if (null != schemaOutputFilename)
      this.schemaOutputFilename = schemaOutputFilename;
    if (null != dbName)
      this.dbName = dbName;
    if (null == format) { throw new IllegalArgumentException(String.format(
        "--schema-output-type must specify a valid OUTPUT_TYPE! [%s]",
        StringUtils.join(OUTPUT_TYPES.values(), ", "))); }

    // Print the schema
    logger.debug("Printing schema as " + format.toString());

    // If it's not specified, assume that the database name should be the
    // sqlfile minus extension
    if (null == this.dbName) {
      StringBuilder dbNameBuilder = new StringBuilder();
      String[] dbFilenamePieces = this.sqlFilename.split("/");
      dbFilenamePieces = dbFilenamePieces[dbFilenamePieces.length - 1]
          .split("\\.");
    ...
    // Build the TABLE and COLUMN namespaces, which depend on the database name
    ...
    this.tableMap = new HashMap<String, RDBTable>(
        ((MySQLListener) listener).tableMap);
      ...
      for (RDBTable table : tableList) {
        ...
        for (RDBColumn pk : table.primary_keys) {
          System.out.printf("  PK%s : %s (%s)%s %s\n",
              (table.foreign_keys.containsKey(pk)) ? ", FK" : "    ", pk.name,
              pk.type.name(), (table.foreign_keys.containsKey(pk)) ? " --> "
                  + table.foreign_keys.get(pk).table.tableName + "."
                  + table.foreign_keys.get(pk).name : "",
        ...
        for (RDBColumn fk : table.foreign_keys.keySet()) {
          if (!table.primary_keys.contains(fk))
            System.out.printf("      FK : %s (%s)%s %s\n", fk.name,
                fk.type.name(), (table.foreign_keys.containsKey(fk)) ? " --> "
                    + table.foreign_keys.get(fk).table.tableName + "."
                    + table.foreign_keys.get(fk).name : "",
        ...
        for (RDBColumn column : table.getColumns()) {
          if (!table.primary_keys.contains(column)
              && !table.foreign_keys.containsKey(column))
            System.out.printf("         : %s (%s) %s\n", column.name,
                (null != column.type) ? column.type.name() : "null",
    ...
    // Output triples in MarkLogic's native sem:triple format
      ...
      try {
        ...
        xMLStreamWriter.writeStartElement(SemTriple.SEM, "triples",
            SemTriple.SEM_TRIPLE_NAMESPACE);
        xMLStreamWriter
            .setPrefix(SemTriple.SEM, SemTriple.SEM_TRIPLE_NAMESPACE);
        xMLStreamWriter.writeNamespace(SemTriple.SEM,
            SemTriple.SEM_TRIPLE_NAMESPACE);
        ...
        for (RDBTable table : tables) {
          ...
          SemTriple.writeTriple(xMLStreamWriter, makeIRI(DB + this.dbName),
              makeIRI(RDB + "hasTable"), makeIRI(TABLE + table.tableName),
              false);

          // Print primary keys
          for (RDBColumn pk : table.primary_keys) {
            logger.debug(String.format("  PK%s : %s (%s)%s", "    ", pk.name,
                pk.type.name(), ""));
            ...
            SemTriple.writeTriple(xMLStreamWriter, makeIRI(TABLE
                + table.tableName), makeIRI(RDB + "hasPrimaryKey"),
                makeIRI(uniqueURI), false);
          }

          // Print foreign keys
          logger.debug(String.format("Table [%s] contains [%d] foreign keys",
              table.tableName, table.foreign_keys.keySet().size()));
          for (RDBColumn fk : table.foreign_keys.keySet()) {
            logger.debug(String.format("      FK : %s (%s)%s", fk.name,
                fk.type.name(), (table.foreign_keys.containsKey(fk)) ? " --> "
                    + table.foreign_keys.get(fk).table.tableName + "."
                    + table.foreign_keys.get(fk).name : ""));
            ...
            String remoteKey = table.foreign_keys.get(fk).table.tableName + "/"
                + table.foreign_keys.get(fk).name;
            String uniqueRemoteURI = getUniqueColumnURI(remoteKey, COLUMN);

            // Relate table to FK
            SemTriple.writeTriple(xMLStreamWriter, makeIRI(TABLE
                + table.tableName), makeIRI(RDB + "hasForeignKey"),
                makeIRI(uniqueLocalURI), false);

            // Relate local key to remote key
            SemTriple
                .writeTriple(xMLStreamWriter, makeIRI(uniqueLocalURI),
                    makeIRI(RDB + "hasForeignKey"), makeIRI(uniqueRemoteURI),
                    false);
          }
          ...
          for (RDBColumn column : table.getColumns()) {
            ...
            logger.debug(String.format("         : %s (%s)", column.name,
                column.type.name()));
            ...
            SemTriple.writeTriple(xMLStreamWriter, makeIRI(TABLE
                + table.tableName), makeIRI(RDB + "hasColumn"),
                makeIRI(uniqueURI), false);

            // Store name of column
            SemTriple.writeTriple(xMLStreamWriter, makeIRI(uniqueURI),
                makeIRI(RDB + "hasLabel"), column.name, false);
          }
        }
        ...
        logger
            .error(
                "ERROR: Creating new SEM TRIPLE XML output file. "
                    + "Unable to write SEM TRIPLE XML file because it was not well-formed.",
                e);

      }
      catch (NullPointerException npe) {
        logger.error(String.format(
            "ERROR: Failed creating Output filename '%s'!\n\n",
            this.schemaOutputFilename), npe);
    ...
    // Everything else
      ...
      logger.error("Unsupported format type: "
          + ((null != format) ? format.toString() : "null"));
    }
  ...
  /**
   * Read in a file as a String. Use this only for relatively short files, since
   * Java Strings, like diamonds, are forever.
   * 
   * @param path
   *          Path and filename of file to read in
   * @param encoding
   *          Whichever encoding you want to read the file using, e.g.
   ...
   * 
   * @param sqlFilename
   *          File name of the SQL schema file to parse.
   * @param dbName
   * @throws IllegalArgumentException
   *           A required argument was null or a stupid value was used.
   * @throws NoSuchFileException
   *           Specified sqlFilename does not exist.
   * @throws IOException
   *           Failed to read specified sqlFilename for some reason.
   */
  public void parseSchema(String sqlFilename) throws IllegalArgumentException,
    ...
    try {
      ...
      if (null == this.sqlFilename)
        throw new IllegalArgumentException(
            "Parameter sqlFilename MUST be specified!");

      // Load SQL schema file
      sql = readFile(this.sqlFilename, Charset.defaultCharset());
    }
    catch (NoSuchFileException nsfe) {
      logger.error(String.format("Specified SQL filename %s does not exist.",
          this.sqlFilename));
      throw nsfe;
    }
    catch (IOException e) {
      logger.error(String.format("Failed to read incoming SQL filename %s",
          this.sqlFilename));
      throw e;
    }

    // Parse the content of the SQL schema file
    parseSchemaString(sql);

    logger.debug("Done mapping the parse tree of [%s] to the data model. ",
        this.sqlFilename);
  }
   ...
   * 
   * @param sqlString
   *          String content of SQL schema file to parse.
   */
  public void parseSchemaString(String sqlString) {
    // Get our lexer
    CreateTableLexer lexer = new CreateTableLexer(new ANTLRInputStream(
        sqlString));
